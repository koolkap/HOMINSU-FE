...
export default function LiveCard({ item }: { item: LiveItem }) {
  return (
    <button className="group w-[220px] shrink-0 text-left sm:w-[260px]">
      <div className="relative">
        <ImageWithSkeleton
          ...
          width={520}
          height={320}
          className="aspect-[4/3] ring-1 ring-white/5 transition-transform duration-300 group-hover:scale-[1.02] group-focus-visible:scale-[1.02] group-hover:ring-signal/50"
        />
      ...
      <p className="mt-0.5 text-xs text-mist-500">{item.channel}</p>
    </button>
  )
}
